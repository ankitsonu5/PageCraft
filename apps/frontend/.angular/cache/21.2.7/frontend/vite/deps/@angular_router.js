import { Title } from "./chunk-7JXVJZCA.js";
import "./chunk-FGKIAOJ7.js";
import "./chunk-G7VINI3R.js";
import {
  ...
  PlatformNavigation,
  ViewportScroller,
} from "./chunk-DBCMNT3W.js";
import { LOCATION_INITIALIZED, PlatformLocation } from "./chunk-7VWQJIIP.js";
import {
...
function defaultUrlMatcher(segments, segmentGroup, route) {
  ...
  if (
    !matchParts(post, segments.slice(segments.length - post.length), posParams)
  ) {
    return null;
  }
  return {
...
function shouldRunGuardsAndResolvers(curr, future, mode) {
  if (typeof mode === "function") {
    return runInInjectionContext(future._environmentInjector, () =>
      mode(curr, future),
    );
  }
  switch (mode) {
...
var ApplyRedirects = class {
  ...
  applyRedirectCommands(
    ...
    return __async(this, null, function* () {
      const redirect = yield getRedirectResult(
        redirectTo,
        currentSnapshot,
        injector,
      );
...
function getRedirectResult(redirectTo, currentSnapshot, injector) {
  ...
  return firstValueFrom(
    wrapIntoObservable(
      runInInjectionContext(injector, () => redirectToFn(currentSnapshot)),
    ),
  );
}
...
function recognize(
  ...
  return mergeMap((t) =>
    __async(null, null, function* () {
      const { state: targetSnapshot, tree: urlAfterRedirects } =
        yield recognize$1(
          injector,
          configLoader,
          rootComponentType,
      ...
      switchMap((overallTransitionState) => {
        ...
        return of(overallTransitionState).pipe(
          switchMap((t) => {
            ...
            if (this.urlHandlingStrategy.shouldProcessUrl(t.rawUrl)) {
              return of(t).pipe(
                ...
                switchMap((value) =>
                  from(
                    overallTransitionState.routesRecognizeHandler
                      .deferredHandle ?? of(void 0),
                  ).pipe(map(() => value)),
                ),
                tap(() => {
          ...
          switchMap((t) => {
            ...
            const deferred =
              overallTransitionState.beforeActivateHandler.deferredHandle;
...
var ReactiveRouterState = class _ReactiveRouterState {
  ...
  updateState() {
    const { fragment, root, queryParams } =
...
var RouterLink = class _RouterLink {
    ...
    if (commandsOrUrlTree == null) {
      ...
      if (isUrlTree(commandsOrUrlTree)) {
        ...
        this.routerLinkInput.set(
          Array.isArray(commandsOrUrlTree)
            ? commandsOrUrlTree
            : [commandsOrUrlTree],
        );
      }
  ...
  _urlTree = computed(
    () => {
      ...
      const shouldTrackParams = (handling) =>
      ...
      return this.router.createUrlTree(routerLinkInput, {
        relativeTo:
          this._relativeTo() !== void 0 ? this._relativeTo() : this.route,
  ...
  processRoutes(injector, routes) {
    ...
    for (const route of routes) {
      ...
      if (route._loadedNgModuleFactory && !route._loadedInjector) {
        route._loadedInjector = route._loadedNgModuleFactory.create(
          injectorForCurrentRoute,
        ).injector;
      }
...
var NavigationStateManager = class _NavigationStateManager extends StateManager {
  ...
  get registered() {
    return (
      this.nonRouterEntryChangeListener !== void 0 &&
      !this.nonRouterEntryChangeListener.closed
    );
  }
  constructor() {
    super();
  ...
  registerNonRouterCurrentEntryChangeListener(listener) {
    ...
    this.nonRouterEntryChangeListener =
      this.nonRouterCurrentEntryChangeSubject.subscribe(({ path, state }) => {
        listener(
          path,
          state,
          "popstate",
  ...
  handleRouterEvent(e, transition) {
    return __async(this, null, function* () {
      ...
      if (e instanceof NavigationStart) {
        ...
        transition.routesRecognizeHandler.deferredHandle = new Promise(
          (resolve) =>
            __async(this, null, function* () {
              if (this.urlUpdateStrategy === "eager") {
                try {
        ...
        transition.beforeActivateHandler.deferredHandle = new Promise(
          (resolve) =>
            __async(this, null, function* () {
  ...
  cancel(transition, cause) {
    return __async(this, null, function* () {
      ...
      if (
        ...
        cause.code === NavigationCancellationCode.Aborted
      ) {
      ...
      if (isTraversalReset) {
        ...
        const internalPath = this.urlSerializer.serialize(
          this.getCurrentUrlTree(),
        );
  ...
  handleNavigate(event) {
    ...
    let scroll = this.inMemoryScrollingEnabled
      ? "manual"
      : (this.currentNavigation.routerTransition?.extras.scroll ??
        "after-transition");
      ...
      this.currentNavigation.commitUrl = () =>
        __async(this, null, function* () {
          ...
          if (transition && !transition.extras.skipLocationChange) {
            ...
            const history =
              this.location.isCurrentPathEqualTo(internalPath) ||
              !!transition.extras.replaceUrl
                ? "replace"
                : "push";
            const state = __spreadProps(
  ...
  eventAndRouterDestinationsMatch(navigateEvent, transition) {
    ...
    return (
      new URL(routerDestination, eventDestination.origin).href ===
      eventDestination.href
    );
  }
